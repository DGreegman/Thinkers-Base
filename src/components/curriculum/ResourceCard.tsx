...
export default function ResourceCard({ resource, user, index, onApprove, onDelete }: Props) {
  const meta        = FILE_TYPES[resource.fileType as keyof typeof FILE_TYPES] || FILE_TYPES.other;
  const canManage   = user.role === "admin" || (user.role === "teacher" && resource.uploadedBy === user.name);
  const hasFile     = Boolean(resource.fileName);
  const downloadUrl = hasFile ? `/uploads/curriculum/${resource.fileName}` : null;
  ...
  return (
    <motion.div
      ...
      className={`bg-white rounded-card shadow-card hover:shadow-card-hover transition-all duration-300 overflow-hidden group ${
        !resource.approved ? "border-2 border-dashed border-orange-200" : "border border-gray-100"
      }`}
    >
      {/* Top accent + file type */}
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-x-3 gap-y-1 mb-4">
          ...
          {resource.fileSize > 0 && (
            <>
              <span className="font-poppins text-charcoal/45 text-xs">·</span>
              <span className="font-poppins text-charcoal/45 text-xs">{formatFileSize(resource.fileSize)}</span>
            </>
          )}
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          {resource.approved ? (
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-poppins text-xs text-orange-500 font-semibold">Pending Approval</span>
            </div>
          )}
        ...
        <p className="font-poppins text-charcoal/40 text-xs mb-4">
          Uploaded by <span className="font-semibold text-charcoal/60">{resource.uploadedBy}</span>
        </p>
